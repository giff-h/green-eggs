...
nox.sessions.SessionRunner._orig_friendly_name = (  # type: ignore[attr-defined]
    nox.sessions.SessionRunner.friendly_name
)
...
def _get_latest_minor_versions() -> List[str]:
    def shell_output(*args):
        return (
            subprocess.run(list(args), stdout=subprocess.PIPE)
            .stdout.decode('utf-8')
            .strip()
        )
    ...
    sorted_versions = sorted(
        tuple(map(int, v.split('.'))) for v in versions_output.split('\n')
    )
    paths = []
    for minor, minor_group in itertools.groupby(
        sorted_versions, operator.itemgetter(1)
    ):
        if minor >= 6:
            latest_minor = tuple(map(str, list(minor_group)[-1]))
            path = os.path.join(
                pyenv_root, 'versions', '.'.join(latest_minor), 'bin', 'python'
            )
            _friendly_name_mapping[
                f'tests_pyenv-{path}'
            ] = f'tests-pyenv-python{".".join(latest_minor[:-1])}'
            paths.append(path)
    return paths
...
def tests_pyenv(session: nox_poetry.Session):
    """
    Runs pytest on the latest patch of each available pyenv minor python version at
    least 3.6.
    """
